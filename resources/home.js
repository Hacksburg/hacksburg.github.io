...
function createCopyFeedbackElement() {
	// Create the single feedback element with standalone styling
	copyFeedbackElement = document.createElement('div');
	copyFeedbackElement.className = 'copy-feedback';
	copyFeedbackElement.textContent = 'Copied link to clipboard';
	document.body.appendChild(copyFeedbackElement);
}

function setUpPostLinkCopy() {
	// Add copy functionality only to title-and-subtitle-wrapper divs within posts that have IDs
	Array.from(document.getElementsByClassName("title-and-subtitle-wrapper")).forEach(wrapperDiv => {
		const postElement = wrapperDiv.closest('.post');
		// Only add copy functionality if the post has an ID (generated by build.py)
		if (postElement && postElement.id) {
			// Change cursor to copy on hover
			wrapperDiv.style.cursor = "copy";
			
			// Add click event to copy link to clipboard
			wrapperDiv.addEventListener('click', function() {
				const currentUrl = window.location.origin + window.location.pathname;
				const linkToCopy = currentUrl + '#' + postElement.id;
...
function showCopyFeedback() {
	...
	copyFeedbackElement.classList.remove('fade-out');
	
	// Show immediately at 100% opacity (no transition)
	copyFeedbackElement.classList.add('show');
	
	// Start fade-out after staying visible for a moment
	setTimeout(() => {
		copyFeedbackElement.classList.remove('show');
		copyFeedbackElement.classList.add('fade-out');
	}, 750); // Stay visible for 0.75s before starting 1.5s fade-out
}
...
function createPastEventsMarker() {
	...
	marker.innerHTML = `<div class="past-text">Past Events</div><div class="past-line"></div>`;
	return marker;
}
